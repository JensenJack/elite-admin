...
<div @include('backend.crud.inc.field_wrapper_attributes') >
	...
	<input
        ...
        @include('backend.crud.inc.field_attributes')
	...
	<div class="btn-group" role="group" aria-label="..." style="margin-top: 3px;">
	  <button type="button" data-inputid="{{ $field['name'] }}-filemanager" class="btn btn-default popup_selector">
		<i class="fa fa-cloud-upload"></i> {{ trans('luismareze::crud.browse_uploads') }}</button>
		<button type="button" data-inputid="{{ $field['name'] }}-filemanager" class="btn btn-default clear_elfinder_picker">
		<i class="fa fa-eraser"></i> {{ trans('luismareze::crud.clear') }}</button>
	</div>
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
	...
	@push('crud_fields_styles')
		...
		<link href="{{ asset('vendor/luismareze/colorbox/example2/colorbox.css') }}" rel="stylesheet" type="text/css" />
	@endpush

	@push('crud_fields_scripts')
		<!-- include browse server js -->
		<script src="{{ asset('vendor/luismareze/colorbox/jquery.colorbox-min.js') }}"></script>
	@endpush
